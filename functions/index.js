   ...
   return new Promise((resolve, reject)=>{
      ...
      chatRef.onSnapshot(snapChat=>{
         ...
         let messageRef = chatRef.collection('messages').doc(messageId).onSnapshot(snapMessage=>{
            ...
            let usetTo = Object.keys(chatDoc.users).filter(key =>{ return (key !== encode64(userFrom)) })[0];

            console.log('[FROM]', userFrom);
            console.log('[TO]'), usetTo;

            db.collection("users").doc(decode64(usetTo)).collection('contacts').doc(encode64(userFrom)).set({
